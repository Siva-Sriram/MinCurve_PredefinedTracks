...
def quadratic_programming_optimization(x, y, num_points, track_width_left, track_width_right):
    ...
    objective = cp.Minimize(10 * cp.sum_squares(y_var[2:] - 2 * y_var[1:-1] + y_var[:-2]) +
                            10 * cp.sum_squares(y_var[1:] - y_var[:-1]) +
                            10 * cp.sum_squares(y_var[-1] - y_var[0]) +
                            1e40 * cp.sum(cp.abs(dx_dt * d2y_dt2 - d2x_dt2 * dy_dt) / (dx_dt**2 + dy_dt**2 + epsilon)**1.5))
...
def main():
    ...
    optimized_x, optimized_y, optimization_time = quadratic_programming_optimization(x_coord_cp, y_coord_cp, num_points, track_width_left,
                                                                  track_width_right)
    ...
    fig.add_trace(
        go.Scatter(x=x_left_boundary, y=y_left_boundary, mode='markers', name='Left Boundary', line=dict(color='black')))
    fig.add_trace(go.Scatter(x=x_right_boundary[::-1], y=y_right_boundary[::-1], mode='markers', name='Right Boundary',
                             line=dict(color='black')))

    x_smooth_opt, y_smooth_opt = zip(*smooth_optimized_track)
    fig.add_trace(go.Scatter(x=x_smooth_opt, y=y_smooth_opt, mode='markers', name='Smooth Optimized Track'))

    x_smooth_centerline, y_smooth_centerline = zip(*smooth_centerline)
    fig.add_trace(go.Scatter(x=x_smooth_centerline, y=y_smooth_centerline, mode='markers', name='Smooth Centerline'))
