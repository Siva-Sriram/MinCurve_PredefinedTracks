...
def smooth_track_with_bezier(track, num_points=100):
    ...
    # Add buffer points for smoothing the transition
    buffer_points = bezier_curve([track[-2], track[-1], track[0]], num_points=num_points//2)
    smooth_track.extend(buffer_points)
...
def main():
    ...
    smooth_optimized_track[-len(smooth_transition_points):] = smooth_transition_points
    ...
    global_curvature_smooth_centerline, points_smooth_centerline, length_smooth_centerline = calculate_global_curvature(
    x_smooth_centerline, y_smooth_centerline)
