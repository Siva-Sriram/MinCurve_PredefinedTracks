...
def quadratic_programming_optimization(x, y, num_points, track_width_left, track_width_right):
    ...
    # Minimize abrupt changes at start and end to ensure a closed loop
    loop_continuity = 100.0 * cp.sum_squares(y_var[1] - y_var[0]) + 100.0 * cp.sum_squares(y_var[-1] - y_var[-2])

    # Penalize curvature changes across the entire track
    d2y_dt2 = y_var[2:] - 2 * y_var[1:-1] + y_var[:-2]
    smoothness = cp.sum_squares(d2y_dt2)

    # Define the complete objective including curvature minimization
    objective = loop_continuity + 1000.0 * smoothness + 10000.0 * cp.sum_squares(d2y_dt2)  # Increase weights for curvature minimization
    ...
    try:
        problem.solve(verbose=True, solver=cp.SCS)  # Use verbose=True and a different solver
    except cp.error.SolverError as e:
        print(f"Solver failed: {e}")
        return x, y, 0  # Return original coordinates if solver fails
    end_time = time.time()
