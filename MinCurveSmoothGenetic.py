...
def smooth_track_with_spline(track, num_points):
    ...
    tck, _ = interp.splprep([x_track, y_track], s=0, per=True)
...
def main():
    num_generations = 250
    population_size = 75
    target_curvature = 1  # Target curvature value
    penalty_weight = 200  # Increased penalty weight for high curvature values
    mutation_rate = 0.05  # Lower mutation rate
    mutation_range = 0.1  # Reduced mutation range

    best_track = genetic_algorithm(num_generations, population_size, target_curvature, penalty_weight)

    # Smooth the best track using cubic spline interpolation
    smooth_best_track = smooth_track_with_spline(best_track, num_points=1000)
